...
START_TEST(overflow_matrix_1_1) {
  ...
  b.matrix[0][0] = DBL_MAX / 10;

  matrix_t res;

  ck_assert_int_eq(CALC_ERROR, s21_sum_matrix(&a, &b, &res));
...
START_TEST(ok_matrix_3_4_null_res) {
  matrix_t a, b;
...
START_TEST(incorrect_matrix) {
  ...
  ck_assert_int_eq(INCORRECT_MATRIX, s21_sum_matrix(NULL, NULL, &res));
  ck_assert_int_eq(INCORRECT_MATRIX, s21_sum_matrix(&a, NULL, &res));
  ck_assert_int_eq(INCORRECT_MATRIX, s21_sum_matrix(NULL, &a, &res));
  ck_assert_int_eq(INCORRECT_MATRIX, s21_sum_matrix(&a, NULL, NULL));
  ck_assert_int_eq(INCORRECT_MATRIX, s21_sum_matrix(&a, &b, NULL));
  ck_assert_int_eq(INCORRECT_MATRIX, s21_sum_matrix(NULL, &b, NULL));
...
Suite *test_s21_sum_matrix_suite() {
  ...
  tcase_add_test(tc, ok_matrix_3_4_null_res);
